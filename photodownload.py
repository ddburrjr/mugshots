...
from dbpackages import db_connect, db_search, db_retrieve
...
def main():
    ...
    scale, counter = 25, 0
    rows = db_retrieve(cur, QUERY_INMATE)
    print(f'{len(rows)} SQL records to process...')
    for row in rows:
        case_id, img_url, img_file = row
        if not db_search(cur, QUERY_PHOTOS.format(case_id)):
            counter += 1
            ...
            original = (img.shape[1], img.shape[0])
